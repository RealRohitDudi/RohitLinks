...
export function validateSEO(metadata: Metadata): { valid: boolean; issues: string[] } {
  const issues: string[] = []
  ...
  if (!metadata.keywords || (Array.isArray(metadata.keywords) && metadata.keywords.length < 3)) {
