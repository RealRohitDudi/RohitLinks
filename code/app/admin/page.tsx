...
import { ADMIN_CREDENTIALS, DEFAULT_LINKS, DEFAULT_PROFILE } from "@/lib/constants"
...
export default function AdminPage() {
  ...
  const [theme, setTheme] = useState<ThemeSettings>({ mode: "dark", accentColor: "cyan" })
  ...
  const handleSaveLink = () => {
    ...
    if (editingLink) {
      updatedLinks = links.map((l) =>
        l.id === editingLink.id
          ? { ...l, title: formData.title.trim(), url: formData.url.trim(), icon: formData.icon }
          : l,
      )
    } else {
  ...
  const handleToggleVisibility = (id: string) => {
    const updatedLinks = links.map((l) => (l.id === id ? { ...l, visible: !l.visible } : l))
    setLinks(updatedLinks)
    saveLinks(updatedLinks)
  }
  ...
  if (!authenticated) {
    return (
      <main className="min-h-screen starfield flex items-center justify-center px-4" role="main">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8 relative z-10">
          <div>
            ...
            <p className="text-muted-foreground">Manage your personal link profile</p>
          </div>
          <button
          ...
          <div className="space-y-8 relative z-10">
            ...
            <div className="bg-card border border-border rounded-xl p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-white">Links ({links.length})</h2>
                <button
              ...
              <div className="space-y-2" role="list">
                {links.length === 0 ? (
                  <p className="text-muted-foreground text-center py-8">No links yet. Add one to get started!</p>
                ) : (
            ...
            <div className="bg-card border border-border rounded-xl p-6">
              <h2 className="text-2xl font-bold text-white mb-4">Backup & Import</h2>
      ...
      {showModal && (
        ...
        >
          <div className="bg-card border border-border rounded-xl p-6 max-w-md w-full">
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="link-title" className="block text-sm font-medium text-foreground mb-2">
                  Title
                </label>
                <input
                  id="link-title"
              ...
              <div>
                <label htmlFor="link-icon" className="block text-sm font-medium text-foreground mb-2">
                  Icon
                </label>
                <select
                  id="link-icon"
